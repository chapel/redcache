...
function ChainCallback () {
  ...
  function composeKV(key, value) {
    chain.keyValues[key] = value
    chain.argKV.push({key: key, value: value})
  }
  ...
  chain.miss = function(miss) {
    chain._miss = function(done) {
      var keys = Object.keys(chain.missedKeys)
      keys = keys.length > 1 ? keys : keys[0]
      miss(keys, composeKV, done)
    }
    return chain
  }
